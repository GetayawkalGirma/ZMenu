...
export default async function PublicMealDetailPage({
  ...
  const preferredHeroImage = restaurants.find((r: any) => r.imageUrl)?.imageUrl || res.imageUrl;
    ...
    <div className="bg-gray-50/50 min-h-screen">
      ...
      <section className="bg-white border-b border-gray-100 py-12 relative overflow-hidden">
        ...
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 relative z-10">
          <Link href="/search" className="inline-flex items-center text-[10px] font-black uppercase tracking-[0.2em] text-gray-400 hover:text-blue-600 transition-colors mb-8">
          ...
          <div className="flex flex-col md:flex-row gap-12 items-center justify-between">
            ...
            <div className="flex-1 space-y-6">
              ...
              <div className="pt-8 border-t border-gray-100 flex flex-wrap items-center gap-8">
                <div>
                   <span className="text-[10px] font-black text-gray-300 uppercase tracking-widest block mb-1">Market Avg</span>
                   <span className="text-2xl font-black text-gray-900">{res.avgPrice ? formatPrice(res.avgPrice) : "—"}</span>
                </div>
                <div className="w-px h-8 bg-gray-100 hidden sm:block" />
                <div>
                   <span className="text-[10px] font-black text-gray-300 uppercase tracking-widest block mb-1">Price Range</span>
                   <div className="text-lg font-black text-gray-900">
                      {res.minPrice ? formatPrice(res.minPrice) : "—"} 
                      <span className="mx-2 text-gray-200 font-medium text-sm">/</span>
                      {res.maxPrice ? formatPrice(res.maxPrice) : "—"}
                   </div>
                </div>
                <div className="w-px h-8 bg-gray-100 hidden sm:block" />
                <div>
                   <span className="text-[10px] font-black text-gray-300 uppercase tracking-widest block mb-1">Citations</span>
                   <span className="text-2xl font-black text-blue-600">{restaurants.length} Spots</span>
                </div>
              </div>

              <div className="flex flex-wrap gap-2 pt-4">
                {meal.tags?.map(tag => (
                  <span key={tag} className="px-3 py-1 bg-gray-50 text-gray-400 rounded-lg text-[9px] font-black uppercase tracking-widest border border-gray-100">
                    # {tag}
                  </span>
                ))}
              </div>
            ...
            <div className="w-full md:w-64 aspect-square rounded-[2rem] overflow-hidden shadow-2xl border-4 border-white bg-gray-50 shrink-0 relative group">
              <img 
                src={preferredHeroImage || "https://placehold.co/600x600?text=Premium+Meal"} 
        ...
        <div className="grid grid-cols-1 xl:grid-cols-2 gap-6">
          {restaurants.length > 0 ? (
            restaurants.map((rm: any) => (
              <Card key={rm.id} className="group overflow-hidden border-0 shadow-sm shadow-gray-200 hover:shadow-xl transition-all duration-300 rounded-3xl bg-white">
                <div className="flex flex-col sm:flex-row relative h-auto sm:h-52">
                   ...
                   <div className="relative w-full sm:w-[35%] h-48 sm:h-auto overflow-hidden shrink-0">
                      ...
                      <img 
                        src={rm.imageUrl || res.imageUrl || "https://placehold.co/600x400?text=Premium+Meal"} 
                      ...
                      <div className="space-y-3">
                         <div className="flex justify-between items-start gap-4">
                            ...
                            <div className="text-right shrink-0">
                               <span className="text-xl font-black text-green-600 tabular-nums">{formatPrice(rm.price)}</span>
                            </div>
                         </div>
                         ...
                         <div className="flex flex-wrap items-center gap-4 py-2 text-[10px] font-bold text-gray-400 uppercase tracking-tight">
                            <div className="flex items-center">
                               ...
                               <span className="truncate max-w-[120px]">{rm.restaurant?.location || "Addis Area"}</span>
                            </div>
                            {rm.spicyLevel > 0 && (
                         ...
                         {rm.ingredients && rm.ingredients.length > 0 && (
                            <div className="flex flex-wrap gap-1 mt-2">
                                {rm.ingredients.slice(0, 4).map((ing: string) => (
                                    <span key={ing} className="px-2 py-0.5 bg-gray-50 text-gray-400 rounded-md text-[8px] font-bold uppercase border border-gray-100">
                                        {ing}
                                    </span>
                                ))}
                                {rm.ingredients.length > 4 && (
                                    <span className="text-[8px] text-gray-300 self-center">+{rm.ingredients.length - 4} more</span>
                                )}
                            </div>
                         )}
                      </div>
