    ...
    return (
      <div className="min-h-screen bg-white flex flex-col items-center justify-center p-4">
        ...
        <p className="mt-4 text-gray-400 font-medium">
          This culinary destination has moved or doesn&apos;t exist.
        </p>
        <Link href="/restaurants" className="mt-12">
          <Button
            variant="outline"
            className="rounded-2xl px-10 h-14 font-black uppercase tracking-widest text-[10px]"
          >
            Back to Directory
          </Button>
        </Link>
  ...
  if (isIframe) {
    ...
    if (srcMatch && srcMatch[1]) {
      ...
      const query = encodeURIComponent(
        `${restaurant.name} ${restaurant.location || ""}`.trim(),
      );
      ...
      <div className="relative h-[40vh] sm:h-[64vh] w-full bg-gray-900 overflow-hidden">
        ...
        <div className="absolute bottom-0 left-0 w-full">
          <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 pb-8 sm:pb-16 text-center sm:text-left">
            <Link
              href="/restaurants"
              className="inline-flex items-center text-[8px] sm:text-xs font-black uppercase tracking-widest text-gray-700 hover:text-blue-600 transition-colors mb-6 sm:mb-12 bg-white/80 backdrop-blur-md px-4 sm:px-6 py-2 sm:py-3 rounded-xl sm:rounded-2xl shadow-xl"
            >
            ...
            <div className="flex flex-col md:flex-row md:items-end justify-between gap-6 sm:gap-12">
              ...
              <div className="flex flex-col sm:flex-row items-center gap-3 sm:gap-4">
                ...
                >
                  <Navigation className="w-3.5 h-3.5 sm:w-4 sm:h-4 mr-2 sm:mr-3" />{" "}
                  Directions
                </a>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-12 gap-8 sm:gap-20">
                ...
                <div className="lg:hidden">
                  <RestaurantMenuFilters
                    trigger={
                      <Button
                        variant="outline"
                        className="h-10 rounded-xl px-4 border-gray-100 font-black uppercase text-[8px] tracking-widest flex items-center gap-2"
                      >
          ...
          <div className="lg:col-span-4 space-y-8 sm:space-y-12">
            ...
            <div className="bg-gray-900 rounded-[2rem] sm:rounded-[3rem] p-6 sm:p-10 text-white shadow-2xl shadow-blue-200/40 relative overflow-hidden group">
              ...
              <div className="relative z-10 space-y-6 sm:space-y-8">
                <div className="space-y-1 sm:space-y-2">
                  <span className="text-[8px] sm:text-[10px] font-black text-blue-400 uppercase tracking-widest">
                    About
                  </span>
                  <h3 className="text-xl sm:text-3xl font-black tracking-tighter leading-tight uppercase">
                    Venue Identity
                  </h3>
                </div>
                ...
                <div className="space-y-4 sm:space-y-6">
                  ...
                  <div className="flex items-center justify-between py-2 sm:py-4 border-b border-white/5">
                    ...
                    <span className="text-[10px] sm:text-sm font-black text-blue-400 uppercase tracking-widest leading-none">
                      {restaurant.noiselevel || "Moderate"} • {restaurant.privacylevel?.replace("_", " ") || "Public"}
                    </span>
                  </div>
                  ...
                  <div className="flex items-center justify-between py-2 sm:py-4 border-b border-white/5">
                    ...
                    <span className="text-sm sm:text-xl font-black text-white">
                      ETB {Math.round(restaurant.menuItems?.reduce((acc: number, item: any) => acc + (item.price || 0), 0) / (restaurant.menuItems?.length || 1)) || "---"}
                    </span>
                  </div>
                  ...
                  <div className="flex items-center justify-between py-2 sm:py-4 border-b border-white/5">
                    ...
                    <span className="text-[10px] sm:text-sm font-black text-white uppercase">
                      {restaurant.sourceInfo?.channelName ? `@${restaurant.sourceInfo.channelName}` : "Direct Upload"}
                    </span>
                  </div>
                </div>
                ...
                <div className="pt-2 sm:pt-4">
                  <div className="p-4 sm:p-6 bg-white/5 rounded-xl sm:rounded-[2rem] border border-white/10 backdrop-blur-sm">
                    <p className="text-[8px] sm:text-[10px] text-gray-400 font-bold uppercase tracking-widest mb-1">
                      ZMenu Registry
                    </p>
                    <p className="text-[10px] sm:text-sm text-gray-300 font-medium leading-relaxed">
                      Listed on our platform since {new Date(restaurant.createdAt).toLocaleDateString("en-US", { month: 'long', year: 'numeric' })}.
                      {restaurant.status === 'PUBLISHED' ? " Verified by our culinary curation team." : " Community-sourced entry."}
                    </p>
                  </div>
                </div>
              </div>
            ...
            <section className="space-y-4 sm:space-y-6">
              <h3 className="text-[8px] sm:text-xs font-black text-gray-400 uppercase tracking-[0.3em] px-2 mb-2 sm:mb-4">
                Features
              </h3>
              <div className="px-2">
