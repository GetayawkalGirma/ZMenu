...
type MobileTab = "library" | "assignments";
...
export default function ImageLibraryClient({
  ...
  return (
    <div className="h-screen bg-gray-50 flex flex-col overflow-hidden">
      {/* ── Header ──────────────────────────────────────────────── */}
      <header className="flex-none bg-white border-b border-gray-100 shadow-sm z-20">
        <div className="flex items-center justify-between px-3 sm:px-5 py-3">
          <div className="flex items-center gap-2 sm:gap-3 min-w-0">
            <button
              ...
              className="p-1.5 rounded-lg hover:bg-gray-100 text-gray-400 hover:text-gray-700 transition-colors flex-none"
            >
              <ChevronLeft className="w-5 h-5" />
            </button>
            <div className="min-w-0">
              <h1 className="font-black text-sm leading-none text-gray-900 uppercase tracking-tight truncate">
                {restaurant.name}
              </h1>
          ...
          <div className="flex items-center gap-1.5 sm:gap-2 flex-none">
            <button
              ...
              className="flex items-center gap-1.5 px-2 sm:px-3 py-1.5 rounded-lg text-xs text-blue-600 hover:text-blue-700 hover:bg-blue-50 transition-colors border border-blue-200 font-bold"
              title="Search Telegram for more images"
            >
              <Send className="w-3.5 h-3.5" />
              <span className="hidden sm:inline">Find on Telegram</span>
            </button>
            <button
              onClick={loadData}
              disabled={refreshing || saving}
              className="flex items-center gap-1.5 px-2 sm:px-3 py-1.5 rounded-lg text-xs text-gray-500 hover:text-gray-800 hover:bg-gray-100 transition-colors border border-gray-200 font-medium disabled:opacity-50"
            >
            ...
            <button
              ...
              disabled={saving}
              className="flex items-center gap-1.5 px-3 sm:px-4 py-1.5 rounded-lg text-xs font-black uppercase tracking-widest bg-gray-900 hover:bg-black text-white transition-colors shadow-lg disabled:opacity-50"
            >
              {saving ? (
      ...
      <div className="flex-1 flex min-h-0">
        ...
        {/* LEFT — Image Pool */}
        <div className={`w-full md:w-[42%] flex flex-col border-r border-gray-200 bg-gray-900 relative ${mobileTab !== "library" ? "hidden md:flex" : "flex"}`}>
          ...
          <div className="flex-1 overflow-y-auto px-3 pb-4">
            {imagePool.length === 0 ? (
              ...
              <div className="grid grid-cols-3 sm:grid-cols-4 md:grid-cols-3 gap-2 pt-1">
                {imagePool.map((image) => {
        ...
        <div className={`flex-1 flex flex-col min-w-0 bg-white ${mobileTab !== "assignments" ? "hidden md:flex" : "flex"}`}>
          <div className="flex-1 overflow-y-auto px-3 sm:px-5 py-4 space-y-6">
            <section>
...
function MealSlotRow({
  ...
  return (
    ...
    >
      ...
      {assignedImageId && (
        <button onClick={(e) => { e.stopPropagation(); onClear(); }} className="flex-none p-1 rounded-md text-gray-300 hover:text-red-500 hover:bg-red-50 sm:opacity-0 sm:group-hover:opacity-100 transition-all" title="Clear assignment">
